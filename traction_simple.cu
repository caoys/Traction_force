...
float max_time = 200.0f;
const float record_itvl = 10.0f;
//intial condition
...
extern __constant__ float d_areaMin = 350.0f; //min-area size 
...
float h_etaRhoA = 2500.0f;
float h_etaRhoM = 200.0f;
...
__global__ void rhoa_right(float *rhoa, float *rhom, float *phi, float x_sin, float x_cos, float *x, float Lx, int m, int n);
...
int main()
{
	...
	for (int steps = 0; steps<(int)(max_time / dt); steps++){
		...
		rhoa_right<<<blocks, threadsperblock>>>(d_RhoAold, d_RhoMold, d_phi_old, h_xsin, h_xcos, d_x, Lx, Nx, Ny);
		rhoa_right<<<blocks, threadsperblock>>>(d_RhoAnew, d_RhoMnew, d_phi_new, h_xsin, h_xcos, d_x, Lx, Nx, Ny);
...
__global__ void rhs_poisson(float *rhs_x, float *rhs_y, float *ptl_force, float *phi, float *dphix, float *dphiy, float *vis_x, float *vis_y, float *act_x, float *act_y, float *rhoA, float *rhoM, float *ux, float *uy, int Nx, int Ny){
	...
	int j = threadIdx.y + blockIdx.y*blockDim.y;
	int index = i + j * Nx;
	rhs_x[index] = ptl_force[index] * dphix[index] + act_x[index] + vis_x[index];
	rhs_y[index] = ptl_force[index] * dphiy[index] + act_y[index] + vis_y[index];
}
...
__global__ void activeForce(float *div_x, float *div_y, int dir, float *phi, float etaRhoA, float etaRhoM, float *rhoA, float *rhoM, float *dphix, float *dphiy, int Nx, int Ny){
	...
	//x-dir
	if(dir == 1){
		div_x[index] = -etaRhoA * d_epsilon * phi[index] * rhoA[index] * dphix[index] * dphix[index] + etaRhoM * phi[index] * (1.0f - rhoA[index]);
	...
	if(dir == 2){
		...
		div_y[index] = -etaRhoA * d_epsilon * phi[index] * rhoA[index] * dphiy[index] * dphiy[index] + etaRhoM * phi[index] * (1.0f - rhoA[index]);
	}
}
